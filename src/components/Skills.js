			...
			<Container>
				...
				<Row className="row-pb-md">
					{data.map((skill) => {
						...
						return <Col key={id} md='3' sm='6' xs='12' className="chart">
							{/* <div className="chart" data-percent={skill.level * 100}><span><strong>{skill.name}</strong>{skill.level * 100}%</span></div> */}
							<Doughnut plugins={chartPlugin} width={4} height={160} options={chartOpts(name,level)} data={setChartData(values)}/>
						</Col>
					})}
				</Row>
			</Container>
